...
> [!NOTE]
> By default, `deck ls` includes presentations from both personal drives and shared drives. To list only presentations from personal drives, use `--support-all-drives=false`.
...
- `sharedDrive` (boolean): Whether to enable access to Google Shared Drives (Team Drives). Default (`true`) enables access to presentations in shared drives. Set to `false` to restrict access to personal drives only. Can also be configured globally in `config.yml`.
...
- **`sharedDrive`** (boolean): Global setting for Google Shared Drives access (default: `true`)
...
### Google Shared Drives (Team Drives) Support

`deck` supports accessing presentations stored in Google Shared Drives (formerly Team Drives). This feature is enabled by default.

#### Command line usage

```console
# List all presentations including shared drives (default)
$ deck ls

# Disable shared drives access
$ deck ls --support-all-drives=false

# Apply changes to a presentation in a shared drive
$ deck apply deck.md --presentation-id xxxxxXXXXxxxxxXXXXxxxxxxxxxx
```

#### Frontmatter configuration

You can control shared drives access per presentation using frontmatter:

```yaml
---
presentationID: xxxxxXXXXxxxxxXXXXxxxxxxxxxx
title: My Team Presentation
sharedDrive: false  # Disable shared drives access for this presentation
---
```

#### Global configuration

Set the default behavior in your configuration file:

```yaml
# ~/.local/share/deck/config.yml
sharedDrive: false  # Disable shared drives access globally
```

#### Configuration precedence

1. Command line flag `--support-all-drives` (highest priority)
2. Frontmatter `sharedDrive` setting
3. Config file `sharedDrive` setting
